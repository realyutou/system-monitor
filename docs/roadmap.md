...
| 4 | **Frontend skeleton + 串接 `/healthz`** | React Testing Library：render `<App />`，斷言畫面出現「Backend: ok」字樣（mock `fetch('/healthz')` 回 200）。先 fail。 | Vite + React + TypeScript 骨架；`useEffect` 內 `fetch('/healthz')`，狀態顯示在頁面上；測試轉綠。 | 抽 `useHealth()` hook；把 fetch URL 收進 `src/lib/api.ts`。 | `npm test -- app` 全綠 + `npm start` 後瀏覽器看到 "Backend: ok" |
