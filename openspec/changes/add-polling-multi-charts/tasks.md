...
## 7. 🏷️ 三張圖加入 visible title heading（reviewer 要求一眼能辨識）

順序 spec → tdd（紅 → 綠 → refactor）。spec 已在 6.4 / 7.0 合併編輯。

- [x] 7.0 `specs/frontend-app/spec.md` 新增 `Dashboard chart components each render a visible title heading` requirement + 4 個 scenario（CpuChart / MemoryChart / DiskChart 各一 + Dashboard 三題齊出）。`design.md` 新增 Decision #12 解釋元件內建固定文字、`<h3>`、`.chartCard` 包裝。`proposal.md` What Changes 補一條。
- [x] 7.1 🔴 Red：
    - `CpuChart.test.tsx` 加 `it('renders a visible CPU title heading')`：`render(<CpuChart data={cpu.idle} ... />)`；`expect(screen.getByRole('heading', { name: /CPU/i })).toBeInTheDocument()`。
    - `MemoryChart.test.tsx` 同樣加 `Memory` heading 斷言。
    - `DiskChart.test.tsx` 同樣加 `Disk` heading 斷言。
    - `Dashboard.test.tsx` 加 `it('renders all three chart title headings')`：成功 stub 三條 endpoint；`waitFor` 後三個 `getByRole('heading', { name: /CPU|Memory|Disk/i })` 都存在。
- [x] 7.2 跑 `npm test -- chart dashboard`，4 個新案例 fail；既有 51 個全綠
- [x] 7.3 🟢 Green：
    - `CpuChart.tsx` / `MemoryChart.tsx` / `DiskChart.tsx` 三檔包成 `<div className={styles.chartCard}><h3 className={styles.chartTitle}>{TITLE}</h3>{原 chart div}</div>`；三檔 `import styles from '../App.module.css'`。
    - `App.module.css` 新增 `.chartCard`（flex column、`align-items: center`、`gap: var(--space-2)`）與 `.chartTitle`（mono、uppercase、letter-spacing 0.24em、`--color-text`、`font-size: 12px`、`font-weight: 400`、`margin: 0`）。
- [x] 7.4 跑 `npm test`，全部全綠（51 + 4 = 55）
- [x] 7.5 瀏覽器 reload，三張圖上方各顯示對應標題；commit `stage 6: add visible title heading to CpuChart / MemoryChart / DiskChart`
